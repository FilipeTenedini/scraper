async function getData(page){
    ...
    for (let i = 0; i < pagesQt; i++){
        ...
        // para que ele pare um pouco em cada página até raspar os dados eu espero que o estilo dessa div mude.
        ...
        const announcements = await page.evaluate(async () => {
            ...
            document.querySelectorAll('tbody tr').forEach(async (item) => {
                ...
                const download = item.querySelectorAll('#tblDocumentosEnviados > tbody > tr:nth-child(3) > td:nth-child(10) > div > a:nth-child(2)').innerText;
                const view = item.querySelectorAll('#tblDocumentosEnviados > tbody > tr:nth-child(3) > td:nth-child(10) > div > a:nth-child(1)').innerText;
                infos.push({
                    fii, categoria,
                    dataRef, dataEnt,
